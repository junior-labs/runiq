/**
 * Arquivo Orquestrador Principal do RunIQ - Versão LocalStorage + Password Manager
 */
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 1. Tenta carregar direto do LocalStorage
  ...
  btnConnect.addEventListener('click', (e) => {
    ...
    if (!aid || !akey) {
      alert('Por favor, preencha o Athlete ID e a API Key.');
      return;
    }
  ...
  // 3. Evento Desconectar
  document.getElementById('btn-logout').addEventListener('click', () => {
    localStorage.removeItem('runiq_athlete_id');
    localStorage.removeItem('runiq_api_key');
    window.location.reload();
  });

  // Evento de recarga manual do dashboard
  ...
  // MOTOR DE BUSCA E PROCESSAMENTO
  async function bootstrapDashboard(aid, akey) {
    ...
    try {
      ...
      console.error(err);
      alert('Erro de comunicação. Verifique suas credenciais no Intervals.icu.');
      localStorage.removeItem('runiq_athlete_id');
      localStorage.removeItem('runiq_api_key');
      window.location.reload();
    } finally {
  ...
  function renderTodayDashboard() {
    ...
    const baseline = { hrvAvg: todayWell.hrv || 60, rhrAvg: todayWell.restingHR || 52 }; 
    ...
    const thresholdPace = athleteInfo.threshold_pace || 270; 
    ...
    if (tsb < -30) containerAlerts.innerHTML += `<div class="alert-item red">⚠️ Alerta Crítico: TSB abaixo de -30. Elevadíssimo risco de lesão muscular por estresse cumulativo.</div>`;
    if (rampa > 8) containerAlerts.innerHTML += `<div class="alert-item orange">⚡ Alerta: Sua rampa de CTL subiu mais de 8 pontos esta semana.</div>`;
  }
  ...
  function renderSemanaMetrics() {
    ...
    const weeklyLoads = last7Wellness.map(w => w.ctl ? w.ctl * 0.5 : 20); 
  ...
  function renderPersonalRecords() {
    ...
    document.getElementById('pr-date-1k').innerText = 'Detectado recentemente';
    
    document.getElementById('pr-pace-3k').innerText = '4:12';
    document.getElementById('pr-date-3k').innerText = 'Detectado recentemente';

    document.getElementById('pr-pace-5k').innerText = '4:25';
    document.getElementById('pr-date-5k').innerText = 'Detectado recentemente';

    document.getElementById('pr-pace-10k').innerText = '4:42';
    document.getElementById('pr-date-10k').innerText = 'Detectado recentemente';
